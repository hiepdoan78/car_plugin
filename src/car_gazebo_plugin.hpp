...
class CarGazeboPlugin : public gazebo::ModelPlugin {
 ...
 private:
  ...
  double steer = 0;
  double velocity = 0;
  const double wheelbase_length = 0.26;
  const double front_wheelbase_width = 0.198;
  const double rear_wheelbase_width = 0.204;
  const double shock_p = 500;
  const double shock_d = 60;
  const double wheel_diameter = .09;
  const double max_speed = 1;
  const double max_turn_angle = 7.0*M_PI/36.0;
  AckermannModel car_model = {0.198, 0.204};
